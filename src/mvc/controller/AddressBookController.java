...
public class AddressBookController {
    ...
    private void openAdd(){   
       view.getMainGUIFrame().setVisible(false);
        ...
        choiceListener = new ActionListener() {
            ...
            //callDAO("Add");
            Boolean flag = false,valflag=false;
   ...
   */
    ...
    public Boolean mobvalid(){
        Boolean valids = false;
        String mobile = contactDetailsPanel.getMobileField().getText();
        if(mobile.length()<10||mobile.length()>10)
            JOptionPane.showMessageDialog(new JFrame(), "number is invalid","Inane error", JOptionPane.ERROR_MESSAGE);
        else 
            valids = true;
        return valids;
    ...
    private void openSearch(){
        String s=JOptionPane.showInputDialog(null,"enter search name","search",1);
