...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    try
    {
        conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="select * from name where addressbook like '' '"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"',address='"+person.getAddress()+"',pincode='"+person.getPincode()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
    }
    catch (SQLException e)
